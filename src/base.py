...
class CFR:
	...
	def vanilla_cfr(self, history: History, i: Player, t: int, pi_0: float, pi_1: float, debug=False): # Works for two players
	...
	def vanilla_cfr_speedup(self, history: History, t: int, pi_0: float, pi_1: float, debug=False): 
		...
		for a in infoSet.actions():
			...
			# Update cumulative strategy values, this will be used to calculate the average strategy at the end
			infoSet.cumulative_strategy[a] += (pi_0 if history.player() == 0 else pi_1) * infoSet.strategy[a] 
	...
	def get_expected_value(self, history: History, player: Player, player_strategy=None, opp_strategy=None):
		"""
	...
	def get_best_response(self, history: History, player: Player, player_strategy=None):
		...
		if player_strategy:
			...
			average_strategy = infoSet.get_average_strategy() # Use the strategy computed if no strategy provided
		...
		sample_opp_history = history + sample_a # TODO: This does not work for Hold'EM? Because some actions, like folding or calling, end the current game stage, and then it's our turn again
		...
		for opp_idx, opp_action in enumerate(opp_actions):
			...
			ev_opp_action = 0
			for idx, a in enumerate(infoSet.actions()): 
				value = self.get_expected_value(history + a, player, average_strategy, opp_strategy=opp_strategy)

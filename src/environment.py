...
class Player(): # This is the POV
	def __init__(self, balance) -> None:
		...
		self.player_balance: int = balance # TODO: Important that this value cannot be modified easily...
		self.current_bet = 0
...
# If self.finished_playing_game_stage = True, we can move to the next game state. This is needed to go around each player and await their decision
		...
		self.small_blind = 1
		self.big_blind = 2
		self.history: HoldEmHistory = HoldEmHistory() # THis will be the history that will be fed into the AI
	...
	def distribute_pot_to_winning_players(self): # Run when self.game_stage = 5
		...
		for idx, player in enumerate(self.players):
			...
			try:
				self.players_balance_history[idx].append(int(player.player_balance - self.new_player_balance))
			except:
	...
	def start_new_round(self):
		...
		# Big Blind
		self.players[((self.dealer_button_position + 1) % len(self.players))].current_bet = self.big_blind

		# Small Blind
		self.players[((self.dealer_button_position + 2) % len(self.players))].current_bet = self.small_blind
	...
	def play_current_stage(self, action: str = ""):
		...
		if self.players[self.position_in_play].is_AI:
			action = self.players[self.position_in_play].place_bet(self) # Pass the Environment as an argument
			self.history += action
